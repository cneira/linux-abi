...
// 01/17/2007 cneirabustos@gmail.com this one is for RMcobol
	case SCO_xstat:
		return report_sco_xstat(&st, bufp);
	}

#if defined(CONFIG_ABI_TRACE)
	abi_trace(ABI_TRACE_API, "sco xstat version %d not supported\n", vers);
#endif
	return -EINVAL;
}
...
#if defined(CONFIG_ABI_TRACE)
	abi_trace(ABI_TRACE_API, "sco fxstat version %d not supported\n", vers);
#endif
	return -EINVAL;
}
