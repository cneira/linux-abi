...
// sco osr5 is also  svr4 
// added SCO_xstat to make RM cobol work 
  enum {SVR4_stat = 1, SVR4_xstat = 2,SCO_xstat = 51};
...
#if defined(CONFIG_ABI_TRACE)
	abi_trace(ABI_TRACE_API, "svr4 fxstat version %d not supported\n", vers);
#endif
	return -EINVAL;
}
static int
